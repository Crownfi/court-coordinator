...
pub fn unfunded_check() {
	...
	// Sanity check
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::Passed);
...
pub fn permissionless_execution_check() {
	...
	assert_eq!(
		...
		vec![
			cosmwasm_std::Event::new("proposal_executed").add_attribute("proposal_id", proposal_id.to_string())
		]
	);
	assert_eq!(
		...
		vec![
			cosmwasm_std::SubMsg::new(
				cosmwasm_std::CosmosMsg::<SeiMsg>::Bank(
...
pub fn different_proposal_types() {
	...
	assert_eq!(
		...
		cosmwasm_std::SubMsg::new(
			cosmwasm_std::CosmosMsg::<SeiMsg>::Wasm(
				cosmwasm_std::WasmMsg::Execute {
					contract_addr: RANDOM_CONTRACT.into(),
					msg: Binary(format!("{{\"transfer\":{{\"recipient\":\"{RANDOM_ACCOUNT_2}\",\"amount\":\"1337\"}}}}").as_bytes().into()),
					funds: vec![]
				}
			)
		)
	...
	assert_eq!(
		...
		cosmwasm_std::SubMsg::new(
			cosmwasm_std::CosmosMsg::<SeiMsg>::Custom(
				SeiMsg::CallEvm {
					value: 0u128.into(),
	...
	assert_eq!(
		...
		cosmwasm_std::SubMsg::new(
			cosmwasm_std::CosmosMsg::<SeiMsg>::Custom(
				SeiMsg::MintTokens { amount: coin(42069, format!("factory/{RANDOM_CONTRACT}/ayylmao")) }
			)
		)
	);
}
...
pub fn only_execute_on_passed_state() {
	...
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::Pending);
	...
	assert!(execute_response.is_err_and(|err| {err.to_string().contains("status should be \"passed\" for this operation")}));

	helpers::advance_time_to_vote_end(&mut env_deps, proposal_id);
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::Passed);
	helpers::advance_time_to_execution_expiry(&mut env_deps, proposal_id);
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::ExecutionExpired);
	...
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::RejectedOrExpired);
	let execute_response = helpers::execute(
		&mut env_deps,
		None,
		CourtExecuteMsg::ExecuteProposal { id: proposal_id }
	...
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::Executed);
	...
	helpers::advance_time_to_vote_end(&mut env_deps, proposal_id);
	assert_eq!(helpers::query_get_proposal(&env_deps, proposal_id).unwrap().unwrap().status, TransactionProposalStatus::Rejected);
...
pub fn prevent_mint_proposal_overflow() {
	let mut env_deps = new_env_and_instantiate(None);
	let vote_denom = helpers::query_denom(&env_deps).unwrap().votes;
	let tokens_to_mint = u128::MAX.div_ceil(10000).saturating_sub(helpers::get_known_vote_supply(&env_deps)) + 1;
	let proposal_id = helpers::execute_create_guaranteed_passing_proposal(
		&mut env_deps,
		vec![
			ProposedCourtMsgJsonable::TokenfactoryMint {
	...
	assert!(execute_response.is_err_and(|err| {err.to_string().contains("cannot safely operate with the amount of new shares minted")}));
	helpers::advance_time_to_execution_expiry(&mut env_deps, proposal_id);
