...
pub fn unfunded_check() {
	...
	helpers::execute_propose_transaction(&mut env_deps, SHARES_HOLDER_ACCOUNT_1, vec![
		ProposedCourtMsgJsonable::SendCoin { to: RANDOM_ACCOUNT_1.into(), denom: "usei".into(), amount: 1337u128.into() }
	], 1200);
	...
	helpers::assert_unfunded_instruction(
		...
		CourtExecuteMsg::Vote { id: 1, vote: CourtUserVoteStatus::Oppose },
	);
}
...
pub fn vote_on_proposals_which_exist() {
	...
	let execute_response = helpers::execute(
		&mut env_deps,
		Some(MessageInfo { sender: Addr::unchecked(SHARES_HOLDER_ACCOUNT_2), funds: vec![] }),
		CourtExecuteMsg::Vote { id: 0, vote: CourtUserVoteStatus::Oppose }
	);
	assert!(execute_response.is_err_and(|err| {
		err.to_string().contains(" 0 does not exist")
	}));
	let execute_response = helpers::execute(
		&mut env_deps,
		Some(MessageInfo { sender: Addr::unchecked(SHARES_HOLDER_ACCOUNT_2), funds: vec![] }),
	...
	assert!(execute_response.is_err_and(|err| {
		err.to_string().contains(" 1 does not exist")
	}));
	let execute_response = helpers::execute(
		&mut env_deps,
		Some(MessageInfo { sender: Addr::unchecked(SHARES_HOLDER_ACCOUNT_2), funds: vec![] }),
		CourtExecuteMsg::Vote { id: 69420, vote: CourtUserVoteStatus::Oppose }
	);
	assert!(execute_response.is_err_and(|err| {
		err.to_string().contains(" 69420 does not exist")
	}));
	...
	let execute_response = helpers::execute(
		&mut env_deps,
		Some(MessageInfo { sender: Addr::unchecked(SHARES_HOLDER_ACCOUNT_2), funds: vec![] }),
		CourtExecuteMsg::Vote { id: 2, vote: CourtUserVoteStatus::Oppose }
	);
	assert!(execute_response.is_err_and(|err| {
		err.to_string().contains(" 2 does not exist")
	}));
}
...
pub fn cannot_vote_past_expiry() {
	...
	assert!(execute_response.is_err_and(|err| {
		err.to_string().contains("status should be \"pending\" for this operation")
	}));
...
pub fn allow_vote_change() {
	...
	assert_eq!(
		...
		Ok(
			CourtUserVoteInfoJsonable { active_votes: 0u128.into(), vote: CourtUserVoteStatus::Abstain }
		)
	);
	assert_eq!(
...
pub fn emits_vote_event() {
	...
	let execute_response = helpers::execute(
		&mut env_deps,
		Some(MessageInfo { sender: Addr::unchecked(SHARES_HOLDER_ACCOUNT_2), funds: vec![] }),
		CourtExecuteMsg::Vote { id: 0, vote: CourtUserVoteStatus::Abstain }
	).unwrap();
	assert_eq!(
		execute_response.events,
		vec![
