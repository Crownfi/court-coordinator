import { isProbablyTxError, makeQueryErrLessFugly, makeTxExecErrLessFugly } from "@crownfi/sei-utils";
...
export async function showError(error: any) {
	const newModal = (() => {
		if (typeof error.name == "string" && typeof error.message == "string") {
			...
			const txErrorParts = makeTxExecErrLessFugly(error.message);
			if (txErrorParts) {
				const {messageIndex, errorSource, errorDetail} = txErrorParts;
			...
			const queryErrorParts = makeQueryErrLessFugly(error.message);
			if (queryErrorParts) {
				const {errorSource, errorDetail} = queryErrorParts;
					return new PopupModalElement({
						heading: "Blockchain Query Error",
						message: errorSource + "\n" +  errorDetail,
						details: "--error details--\n" +
							"name: " + error.name + "\n" +
							"message: " + (error.message + "").replace(/\t/g, "    ") + "\n" +
