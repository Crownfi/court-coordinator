...
function getCourtConfigModalTemplate(): HTMLTemplateElement {
	if (_templateCourtConfigModal == null) {
		 ...
		 _templateCourtConfigModal.innerHTML = "\n\t<h1>Change configuration</h1>\n\t<form method=\"dialog\" cewt-ref=\"form\">\n\t\t<label>\n\t\t\t<span>Minimum votes for new proposal (as percentage of total supply)</span>\n\t\t\t<input name=\"minimum_vote_proposal_percent\" min=\"0\" max=\"100\" placeholder=\"unchanged\" type=\"number\">\n\t\t</label>\n\t\t<label>\n\t\t\t<span>Minimum voter turnout (percentage)</span>\n\t\t\t<input placeholder=\"unchanged\" max=\"100\" name=\"minimum_vote_turnout_percent\" type=\"number\" min=\"0\">\n\t\t</label>\n\t\t<label>\n\t\t\t<span>Minimum proposal approval rating (percentage)</span>\n\t\t\t<input name=\"minimum_vote_pass_percent\" max=\"100\" placeholder=\"unchanged\" min=\"0\" type=\"number\">\n\t\t</label>\n\t\t<label>\n\t\t\t<span>Maximum proposal voting time</span>\n\t\t\t<input title=\"Enter duration (e.g., 5w4d3h2m1s for 5 weeks, 4 days, 3 hours, 2 minutes, 1 second).\" pattern=\"^\\s*(\\d+w)?\\s*(\\d+d)?\\s*(\\d+h)?\\s*(\\d+m)?\\s*(\\d+s)?\\s*(\\d+ms)?\\s*$\" placeholder=\"unchanged\" name=\"max_proposal_expiry_time_seconds\" type=\"text\">\n\t\t</label>\n\t\t<label>\n\t\t\t<span>Proposal execution window</span>\n\t\t\t<input name=\"execution_expiry_time_seconds\" type=\"text\" pattern=\"^\\s*(\\d+w)?\\s*(\\d+d)?\\s*(\\d+h)?\\s*(\\d+m)?\\s*(\\d+s)?\\s*(\\d+ms)?\\s*$\" title=\"Enter duration (e.g., 5w4d3h2m1s for 5 weeks, 4 days, 3 hours, 2 minutes, 1 second).\" placeholder=\"unchanged\">\n\t\t</label>\n\t\t\n\t\t<div class=\"button-row equal-width\">\n\t\t\t<button class=\"primary small\">Apply changes</button>\n\t\t\t<button class=\"small\" cewt-ref=\"cancel-btn\">Cancel</button>\n\t\t</div>\n\t</form>\n";
	}
